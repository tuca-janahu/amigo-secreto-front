...
export function RestrictionsSection({ groupId, editable }: Props) {
  ...
  return <section className={panelClass} aria-labelledby="restrictions-title">
    ...
    {editable && <Form className="mt-6 grid gap-4 border-t-2 border-dashed border-black pt-5" onSubmit={handleSubmit((data) => create.mutate(data))}>
      ...
      <Form.Label className="flex cursor-pointer items-start gap-3 text-sm normal-case tracking-normal"><Form.Input className="mt-0.5" type="checkbox" {...register('bilateral')} /><span>Fulano e Beltrano não podem se tirar <span className="block text-xs font-normal text-black/60">Desmarcado: apenas Fulaninho não pode tirar Beltraninho.</span></span></Form.Label>
      ...
      <Form.Button className="justify-self-start" variant="tertiary" type="submit" disabled={create.isPending || (participants.data?.length ?? 0) < 2}>{create.isPending ? 'Adicionando...' : 'Adicionar restrição'}</Form.Button>
    </Form>}
