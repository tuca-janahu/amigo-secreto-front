...
export function DashboardPage() {
  ...
  return (
    <main className="min-h-screen bg-amber-100/50 px-5 py-6 font-mono text-black sm:px-8 lg:px-14">
      <header className="mx-auto flex max-w-6xl items-center justify-between gap-4"><PixelBrand /><button className="cursor-pointer border-2 border-black bg-white px-4 py-2 text-xs font-black uppercase shadow-[3px_3px_0_#151515] transition enabled:hover:bg-red-600 hover:text-white disabled:cursor-wait disabled:opacity-60" type="button" onClick={handleLogout} disabled={isLeaving}>{isLeaving ? 'Saindo...' : 'Sair'}</button></header>
      <section className="mx-auto max-w-6xl py-16" aria-labelledby="dashboard-title">
        <p className="inline-block border-2 border-black bg-amber-400 px-3 py-1 text-xs font-black uppercase tracking-wider shadow-[3px_3px_0_#151515]">Area do jogador</p>
        <h1 id="dashboard-title" className="mt-6 max-w-3xl text-4xl leading-[0.95] font-black tracking-[-0.07em] sm:text-6xl">Olá, {displayName}</h1>
        <div className="mt-10 grid gap-5 md:grid-cols-[1.2fr_0.8fr]">
          <section className="border-2 border-black bg-white p-6 shadow-[6px_6px_0_#151515] sm:p-8"><span className="inline-block border-2 border-black bg-amber-400 px-2 py-1 text-xs font-black text-white">EM BREVE</span><h2 className="mt-5 text-2xl font-black uppercase">Seus grupos vão aparecer aqui.</h2><p className="mt-3 max-w-lg text-sm leading-6 text-black/70">Quando a criação de grupos estiver disponível, este será o seu painel para acompanhar cada sorteio.</p></section>
