  ...
  return <main className="min-h-screen bg-green-100 px-4 py-5 font-mono text-black sm:px-8 sm:py-8">
    ...
    <div className="mx-auto max-w-3xl py-8 sm:py-12">
      ...
      <section className={`${panelClass} mt-8 text-center`} aria-live="polite">
        {reveal.isPending ? <div className="reveal-shake py-8"><div className="mx-auto grid size-24 place-items-center border-2 border-black bg-red-600 text-5xl font-black text-white shadow-[5px_5px_0_#151515]">?</div><p className="mt-6 font-black uppercase tracking-wider">Misturando os nomes...</p></div> : showResult ? <div className="py-5"><p className="text-sm font-black uppercase tracking-widest">Você tirou</p><p className="mt-4 break-words text-4xl font-black uppercase text-red-600 sm:text-6xl">{result}</p><button className={`${secondaryButton} mt-7`} type="button" onClick={() => setHidden(true)}>Esconder novamente</button></div> : <div className="py-5"><div className="mx-auto grid size-20 place-items-center border-2 border-black bg-amber-400 text-4xl font-black shadow-[5px_5px_0_#151515]">?</div><h2 className="mt-6 text-xl font-black">Seu amigo secreto ainda está escondido.</h2><button className={`${primaryButton} mt-6 px-6 py-3`} type="button" disabled={reveal.isPending} onClick={() => reveal.mutate()}>{result || access.data.revealed ? 'Rever' : 'Revelar'}</button></div>}
        {reveal.isError && <p className="mt-4 border-2 border-red-700 bg-red-50 p-3 text-sm font-bold text-red-700" role="alert">{getErrorMessage(reveal.error, 'Não foi possível revelar. Tente novamente.')}</p>}
      </section>

      <section className={`${panelClass} mt-8`} aria-labelledby="wall-title"><p className="text-xs font-black uppercase tracking-wider text-red-600">Sem nomes, sem pistas</p><h2 id="wall-title" className="mt-2 text-2xl font-black uppercase">Mural anônimo</h2><form className="mt-6" onSubmit={messageForm.handleSubmit((data) => createMessage.mutate(data))} noValidate><label className="text-xs font-black uppercase" htmlFor="anonymous-message">Nova mensagem</label><textarea id="anonymous-message" className={`${inputClass} mt-2 min-h-28 resize-y`} maxLength={500} {...messageForm.register('content')} aria-invalid={Boolean(messageForm.formState.errors.content)} aria-describedby={messageForm.formState.errors.content ? 'message-error' : undefined} />{messageForm.formState.errors.content && <p id="message-error" className="mt-2 text-xs font-bold text-red-700" role="alert">{messageForm.formState.errors.content.message}</p>}{createMessage.isError && <p className="mt-3 text-sm font-bold text-red-700" role="alert">{getErrorMessage(createMessage.error, 'Não foi possível publicar a mensagem.')}</p>}<button className={`${primaryButton} mt-4`} disabled={createMessage.isPending}>{createMessage.isPending ? 'Enviando...' : 'Enviar anonimamente'}</button></form>
